...
export class ScrapingService {
  ...
  async execute(
    ...
    // -----------------------------------------------------------
    // ✅ CDP – Captura do JSON /pje-consulta-api/api/processos
    // -----------------------------------------------------------
    const initCDP = async (pg) => {
      this.logger.log('🔧 Inicializando CDP para monitoramento de rede...');
      const client = await pg.target().createCDPSession();
        ...
        void (async () => {
          try {
            ...
            if (
              ...
              this.logger.log(`📥 Tentando capturar JSON do processo: ${url}`);
    ...
    try {
      // -----------------------------------------------------------
      // ✅ Seleção da URL base — TRT (1/2) | TST (3)
      // -----------------------------------------------------------
      const isTST = instanceIndex === 3;
      ...
      await retry(
        async () => {
          ...
          await page.type('#nrProcessoInput', processNumber, { delay: 40 });
          await page.click('#btnPesquisar');
        },
        3,
        1000,
      ...
      this.logger.log('✅ Número do processo enviado');

      // -----------------------------------------------------------
      // ✅ FLUXO TST (instância 3)
      // -----------------------------------------------------------
      if (isTST) {
        this.logger.log(
          '✅ Instância 3 detectada — fluxo específico do TST (sem painel)',
        );

        const captchaVisible = await page
          .waitForSelector('#imagemCaptcha', { visible: true, timeout: 8000 })
          .catch(() => null);

        if (!captchaVisible) {
          return {
            process: { mensagemErro: 'Falha ao abrir CAPTCHA no TST' },
            integra: null,
            singleInstance: true,
          };
        }

        this.logger.log('✅ CAPTCHA do TST detectado — resolvendo...');
        ...
        await retry(
          async () => {
            ...
            const base64 = (srcVal as string).replace(
              /^data:image\/\w+;base64,/,
              '',
            );
            const solved = await this.captchaService.resolveCaptcha(base64);
            ...
            await page.type('#captchaInput', solved.resposta, { delay: 40 });
            await page.click('#btnEnviar');
          ...
          3,
          1000,
          'Resolver CAPTCHA TST',
        );

        this.logger.log('✅ CAPTCHA do TST resolvido');
      }

      // -----------------------------------------------------------
      // ✅ FLUXO TRT (instâncias 1 e 2)
      // -----------------------------------------------------------
      else {
        this.logger.log('✅ TRT — verificando se há múltiplas instâncias');
        ...
        if (resultado === 'painel') {
          this.logger.log('✅ Painel de instâncias encontrado');
          ...
          if (!processos.length) {
            return {
              process: { mensagemErro: 'Nenhuma instância encontrada' },
              integra: null,
              singleInstance: false,
            };
          }
          ...
          this.logger.log('✅ Instância selecionada — indo ao CAPTCHA');
        }

        // TRT → uma única instância
        else if (resultado === 'captcha') {
          this.logger.log('✅ Processo possui apenas uma instância no TRT');
          ...
          return {
            process: { mensagemErro: 'Falha ao carregar painel do TRT' },
            integra: null,
            singleInstance: false,
          };
        }

        // CAPTCHA TRT (instâncias 1 ou 2)
        const captchaVisible = await page
          .waitForSelector('#imagemCaptcha', { visible: true, timeout: 8000 })
          .catch(() => null);

        if (captchaVisible) {
          this.logger.log('✅ CAPTCHA do TRT encontrado — resolvendo...');
          ...
          await retry(
            async () => {
              ...
              await page.$eval('#captchaInput', (el: any) => (el.value = ''));
              await page.type('#captchaInput', solved.resposta, {
                delay: 40,
              });
            ...
            3,
            1000,
            'Resolver CAPTCHA TRT',
          );

          this.logger.log('✅ CAPTCHA do TRT resolvido');
        }
      }
      // -----------------------------------------------------------
      // ✅ Captura do processo via CDP
      // -----------------------------------------------------------
      this.logger.log('⏳ Aguardando captura do processo pelo CDP...');
      ...
      // -----------------------------------------------------------
      // ✅ Download do PDF Integra (opcional)
      // -----------------------------------------------------------
      if (downloadIntegra) {
        this.logger.log('📄 Download do PDF /integra solicitado');
      ...
      return {
        process: !downloadIntegra ? capturedResponseData : undefined,
        integra: integraBuffer ?? undefined,
        singleInstance: instanceIndex === 1,
      };
    } finally {
      this.logger.log('♻ Limpando recursos e liberando contexto...');
